...
const char* data_file_path = "/data/nn_data";
...
static det_status_t check_input_param(input_image_t imageData, det_model_type modelType)
{
	...
	if (NULL == imageData.data) {
		ALOGD("Data buffer is NULL");
		_SET_STATUS_(ret, DET_STATUS_PARAM_ERROR, exit);
	}

	if (imageData.pixel_format != PIX_FMT_NV21 && imageData.pixel_format != PIX_FMT_RGB888) {
		ALOGD("Current only support RGB888 and NV21");
		_SET_STATUS_(ret, DET_STATUS_PARAM_ERROR, exit);
	}

	int width,height,channel;
	ret = det_get_model_size(modelType, &width, &height, &channel);
	ALOGD("det_get_model_size width:%d height:%d channel:%d",width,height,channel);
	if (ret) {
		ALOGD("Get_model_size fail!");
		_SET_STATUS_(ret, DET_STATUS_PARAM_ERROR, exit);
	}

	if (width != imageData.width) {
		ALOGD("Inputsize not match! net VS img is width:%dvs%d \n",
	...
	if (imageData.pixel_format == PIX_FMT_RGB888) {
		if (channel != imageData.channel || height != imageData.height) {
			ALOGD("Inputsize not match! net VS img is height:%dvs%d, width:channel:%dvs%d \n", height, imageData.height, channel, imageData.channel);
			_SET_STATUS_(ret, DET_STATUS_PARAM_ERROR, exit);
		}
	}
...
static void check_and_set_dev_type()
{
	...
	if (!fp) {
		ALOGD("Popen /proc/cupinfo fail,set default RevA");
		g_dev_type = DEV_REVA;
		return ;
	}
	...
	ALOGD("Read Cpuinfo:\n%s\n",buffer);
	ALOGD("290 index=%d",index);
	switch (buffer[index+3]) {
		case 'a':
			ALOGD("set_dev_type REVA and setenv 0");
			g_dev_type = DEV_REVA;
			setenv("DEV_TYPE", "0", 0);
			break;
		case 'b':
			ALOGD("set_dev_type REVB and setenv 1");
			g_dev_type = DEV_REVB;
			setenv("DEV_TYPE", "1", 0);
			break;
		default:
			ALOGD("set_dev_type DEV_MS1 and setenv 2 index=%d char:%c", index, buffer[index+3]);
			g_dev_type = DEV_MS1;
			setenv("DEV_TYPE", "2", 0);
			break;
	}
...
{
	ALOGD("Enter, dlopen so:%s", so_file_name[modelType]);
	...
	if (NULL == net->handle_id) {
		ALOGD("dlopen %s failed!", so_file_name[modelType]);
	...
	ret = DET_STATUS_OK;
exit:
	ALOGD("Leave, dlopen so:%s, ret=%d", so_file_name[modelType], ret);
	return ret;
}
...
det_status_t det_set_model(det_model_type modelType)
{
    ...
    printf("det_set_model ############ \n");
	ALOGD("jason jason det_set_model ############ modelType : %d ",modelType);
	int ret = DET_STATUS_OK;
	p_det_network_t net = &network[modelType];
	...
	if (ret) {
		ALOGD("ModelType so open failed or Not support now!!");
		_SET_STATUS_(ret, DET_STATUS_ERROR, exit);
	}
	...
	if (ret) {
		ALOGD("Model_create fail, file_path=%s, dev_type=%d", data_file_path, g_dev_type);
	...
	ALOGD("input_ptr size=%d, addr=%x", width * height * channel * sizeof(uint8_t), net->input_ptr);
exit:
	ALOGD("Leave, modeltype:%d", modelType);
	return ret;
}
...
{
	ALOGD("Enter, modeltype:%d", modelType);
	int ret = DET_STATUS_OK;
	p_det_network_t net = &network[modelType];
	if (!net->status) {
		ALOGD("Model has not created! modeltype:%d", modelType);
		_SET_STATUS_(ret, DET_STATUS_ERROR, exit);
	}

	ret = check_input_param(imageData, modelType);
	if (ret) {
		ALOGD("Check_input_param fail.");
		_SET_STATUS_(ret, DET_STATUS_PARAM_ERROR, exit);
	}
...
det_status_t det_get_result(pDetResult resultData, det_model_type modelType)
{
	...
	if (NETWORK_PREPARING != net->status) {
		ALOGD("Model not create or not prepared! status=%d", net->status);
		_SET_STATUS_(ret, DET_STATUS_ERROR, exit);
	}

	ret = net->process.model_getresult(resultData, net->input_ptr);
	if (ret) {
		ALOGD("Process Net work fail");
