...
#define LOG_TAG "yoloface_process"
...
static void get_detections_result(pDetResult resultData, int num, float thresh, box *boxes, float **probs,  int classes)
{
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (prob > thresh) {
            ...
            resultData->point[detect_num].point.rectPoint.left = boxes[i].x;//left;
            resultData->point[detect_num].point.rectPoint.top = boxes[i].y;//top;
            resultData->point[detect_num].point.rectPoint.right = boxes[i].w;//right;
            resultData->point[detect_num].point.rectPoint.bottom = boxes[i].h;//bot;
            resultData->result_name[detect_num].lable_id = classId;
            detect_num ++;
        }
    }
    
    ALOGD("yoloface postprocess, detect_num:%d ", detect_num);
